...
@section('page-header', 'Video 2')
@section('page-description', '// Index From Video 2 ...')
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="text-nowrap align-middle text-center">
          @if ( $item->active == 1 ) <a href="/dashboard/setups/video-1/nonactive/{{ $item->id }}"><span class="label label-success"> Yes </span></a> @endif
          @if ( $item->active == 2 ) <a href="/dashboard/setups/video-1/active/{{ $item->id }}"><span class="label label-inverse"> No </span></a> @endif
        </td>
        <td class="text-nowrap align-middle text-center">
          <button type="button" class="btn btn-inverse btn-sm"> Edit </button>
          <button type="button" class="btn btn-inverse btn-sm" data-toggle="modal" data-target="#ModalPreviewVideo{{$item->id}}"> Preview </button>
          <form method="POST" action="{{ url('dashboard/setups/video-1' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
            {{ method_field('DELETE') }}
            {{ csrf_field() }}
