...
<body class="mtheme_featured-template-default single single-mtheme_featured postid-7315 woocommerce-no-js single-author demo middle-logo page-is-fullscreen fullscreen-video-type-html5 fullscreen-video fullscreen-eventbox-inactive no-title-background theme-fullwidth body-dashboard-push footer-is-on">
  ...
  <div id="backgroundvideo">
    @if ( count($video2) != false )
    @foreach ( $video2 as $item )
    <video id="video2" class="video-js vjs-default-skin" preload="auto" width="100%" src="EXILEDNONAME/files/{{ $item->file }}" autoplay></video>
    @endforeach
    @else
    <video id="video2" class="video-js vjs-default-skin" preload="auto" width="100%" src="EXILEDNONAME/files/404.mp4" autoplay muted></video>
    @endif
  </div>
</div>
